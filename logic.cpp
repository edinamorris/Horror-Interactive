...
struct UserData
{
    ...
    /*EDITED*/
    //certain actions mean the doll will be waititng for player
    bool dollwaiting = false;

    /*EDITED*/
    //flag for secret playroom door
    bool playRoomSecretDoor = false;

    /*EDITED*/
    //flag for whether bone has been picked up
    bool bonePickedUp = false;
};
...
UserData F_Kitchen(UserData Player)
{
    ...
    while (!KITC_explore)
    {
        ...
        switch (KITC_UsrDecision)
        {
            ...
            {
                /* EDITED*/
                //added the option for taking bone out of bodies pocket
                // as long as bone hasnt already been picked up then show these options
                if(Player.bonePickedUp==false)
                {
                    std::cout << "The body is old and decaying, you see a bone in the pocket do you want to pick it up?" << std::endl;
                    std::cout << "(1) Yes or (2) No" << std::endl;
                    std::cin >> KITC_UsrDecision;
                    switch (KITC_UsrDecision)
                    {
                        case 1:
                            std::cout << "You pick up the bone" << std::endl;
                            Player.bonePickedUp=true;
                            break;

                        case 2:
                            std::cout << "You leave the dead body" << std::endl;
                            break;
            ...
            case 3:
            {
                ...
                else if(Player.bonePickedUp==false)
                {
                    std::cout << "The dog could have used a bone..." << std::endl;
                    break;
                }
                break;
            }
...
UserData F_Playroom(UserData Player)
{
    ...
    while (!PLAY_explore)
    {
        ...
        /*EDITED*/
        //only show this option if the playroom hasnt been discovered already
        if(Player.playRoomSecretDoor==false)
        {
            std::cout << "(4) Explore area around crib"<< std::endl;
        }
        std::cout << "(5) Explore elsewhere" << std::endl;

        //If the player has found the teddy in the corridor give them an option
        //to do something with it
        if (Player.teddy == true)
        {
            std::cout << "(6) Put the teddy you found earlier back" << std::endl;
        }
        ...
        {
            case 1:
                std::cout << "The box opens and starts to play music, you feel safer" << std::endl;
                break;
            ...
            case 3:
                ...
                break;
            /*EDITED*/
            //add in secret back entrance, mainly to be used if you dont break the toy, doll will only be waiting at front door not back
            case 4:
                std::cout << "You explore by the crib and find a secret back door leading to the corridor" << std::endl;
                Player.playRoomSecretDoor=true;
                break;
            case 5:
                std::cout << "You've chosen to explore elsewhere" << std::endl;
                /*EDITED*/
                //added this option if player wants to leave by secret entrance (maybe avoiding the doll), doll will no longer be waiting
                if(Player.playRoomSecretDoor==true)
                {
                    std::cout << "do you want to leave by the secret entrance?" << std::endl;
                    std::cout << "(1) Yes or (2) No" << std::endl;
                    std::cin >> PLAY_UsrDecision;
                    ...
                    {
                        case 1:
                            std::cout << "You leave by the secret entrance and arrive in the corridor" << std::endl;
                            Player.dollwaiting=false;
                            break;

                        case 2:
                            std::cout << "You leave by the main entrance" << std::endl;
                            break;
                ...
                //added this option in, if player doesnt knock off humpty dumpty and find the key then the doll will be watitng at main door
                if(Player.dollwaiting==true)
                {
                    std::cout << "You failed to break humpty the doll was waiting for you!!" << std::endl;
                    Player.alive = false;
                    return Player;
                }
                PLAY_explore = true;
            ...
            case 6:
                //putting the teddy back should reward you with another room key, but as there are more room keys than rooms maybe we should
                //keep this as an exit key as well???
...
int main()
{
        ...
        if (PersonPlaying.alive)
        {
            ...
            else
            {
                do
                {
                    ...
                    {
                    //edited for easier user use, lower and capital letters
                        case 'C':
                        case 'c':
    ...
    //edited to make code run, needs to return int
    return 0;
